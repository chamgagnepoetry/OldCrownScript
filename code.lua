...
BOTTOMTEXT.Position = UDim2.new(0.5, 0,0.83, 5)
BOTTOMTEXT.Size = UDim2.new(0, 100, 0, 15)
BOTTOMTEXT.Font = Enum.Font.Code
BOTTOMTEXT.Text = ""
BOTTOMTEXT.TextColor3 = Color3.fromRGB(226, 226, 226)
BOTTOMTEXT.TextScaled = true
BOTTOMTEXT.TextSize = 14.000
BOTTOMTEXT.TextWrapped = true
BOTTOMTEXT.Visible = false
UISTROKE5.Parent = BOTTOMTEXT
...
local function processQueue(Type,gunName)
	local equippedTool
	...
	local initialValue
	if Type == "Ammo" and gunName and InventoryFolder:FindFirstChild(gunName) then
		initialValue = InventoryFolder[gunName].Value
	end
	...
	while #itemsToPurchase > 0 do
				...
				if string.find(toolName,'[High-Medium Armor]') then
					if Character:FindFirstChild("BodyEffects") and Character.BodyEffects:FindFirstChild("Armor") and Character.BodyEffects.Armor.Value < 130 then
						while Character:FindFirstChild("BodyEffects") and Character.BodyEffects:FindFirstChild("Armor") and Character.BodyEffects.Armor.Value < 130 and Toggles.AutoArmor.Value do
							actionText.Text = 'purchasing...high medium armor'
							Humanoid:UnequipTools()
							HumanoidRootPart.CFrame = item.Head.CFrame * CFrame.new(0,3,0)
							task.wait(0.09)
							fireclickdetector(item.ClickDetector)
				...
				elseif string.find(toolName,'[Medium Armor]') then
					if Character:FindFirstChild("BodyEffects") and Character.BodyEffects:FindFirstChild("Armor") and Character.BodyEffects.Armor.Value < 100 then
						while Character:FindFirstChild("BodyEffects") and Character.BodyEffects:FindFirstChild("Armor") and Character.BodyEffects.Armor.Value < 100 and Toggles.AutoArmor.Value do
							actionText.Text = 'purchasing...medium armor'
							Humanoid:UnequipTools()
							HumanoidRootPart.CFrame = item.Head.CFrame * CFrame.new(0,3,0)
							task.wait(0.09)
							fireclickdetector(item.ClickDetector)
		...
		elseif item and item.Parent and Type and gunName then
			if Type == "Ammo" then
				local attempts = 0
				local maxAttempts = 25
				while InventoryFolder[gunName].Value <= initialValue and attempts < maxAttempts do
					Humanoid:UnequipTools()
					HumanoidRootPart.CFrame = item.Head.CFrame * CFrame.new(0,3,0)
					task.wait(0.09)
					fireclickdetector(item.ClickDetector)
...
local function voidDesync()
	...
	local FakeCFrame = HumanoidRootPart.CFrame
	local CurrentTime = tick()
	local farPosition = Vector3.new(
		math.random(-10000000000000, 10000000000000),
		math.random(10000000000000 * 5, 10000000000000 * 10),
		math.random(-10000000000000, 10000000000000)
	)
	FakeCFrame = CFrame.new(farPosition) 
	HumanoidRootPart.CFrame = FakeCFrame
	RunService.RenderStepped:Wait()
	HumanoidRootPart.CFrame = SavedCFrame
	--camPart.CFrame = SavedCFrame * CFrame.new(0, 2.5, 0)
end
...
local function isPlayerBlacklisted(targetPlayer)
	if not KillAuraBlacklistTable then return false end
	for _, name in ipairs(KillAuraBlacklistTable) do
		if targetPlayer.Name == name or targetPlayer.DisplayName == name then
			return true
		end
	end
...
local function shouldKill(target)
	local hasBlacklist = KillAuraBlacklistTable and #KillAuraBlacklistTable > 0
	local hasWhitelist = KillAuraWhitelistTable and #KillAuraWhitelistTable > 0
	if hasBlacklist then
		return isPlayerBlacklisted(target)
	elseif hasWhitelist then
		return not isPlayerWhitelisted(target)
	end
	return true
end
...
local function GetClosestPlayerToRadius(wallcheck)
	local closestPlayer = nil
	local shortestDistance = math.huge
...
local function createESP(target)
	...
	if ESPGUICLONE then
		...
		if Options.ESPDisplayType.Value == 'username' then
			...
			ESPGUICLONE.PLAYERNAME.Text = target.DisplayName
		end
		ESPGUICLONE.PLAYERNAME.Visible = true
		ESPGUICLONE.BOTTOMTEXT.Visible = Toggles.ESPBottomText.Value
		...
		ESPGUICLONE.PLAYERNAME.TextColor3 = Options.NameTextColor.Value
		ESPGUICLONE.Armor.BackgroundColor3 = Options.ArmorBarColor.Value
		ESPGUICLONE.Health.BackgroundColor3 = Options.HealthBarColor.Value
		ESPGUICLONE.BOXUI.UIStroke.Color = Options.BoxColor.Value
		ESPGUICLONE.BOTTOMTEXT.TextColor3 = Options.BottomTextColor.Value
		...
		local function updateESP(character)
			...
			if hrp then
				...
				local function updateBottomText()
					local BottomText = ESPGUICLONE:FindFirstChild("BOTTOMTEXT")
					if not BottomText then return end
					if Options.ESPBottomText.Value == "distance" then
						if character and character:FindFirstChild("HumanoidRootPart") and hrp then
							local dist = (character.HumanoidRootPart.Position - HumanoidRootPart.Position).Magnitude
							BottomText.Text = '[' .. math.floor(dist) .. '] studs'
						end
					else
						local currentTool = GetEquippedTool(character)
						if currentTool then
							BottomText.Text = currentTool.Name
						else
							BottomText.Text = '[None]'
						end
					end
				end
				bottomConnection = RunService.Heartbeat:Connect(updateBottomText)
			end
		end
...
local function updateESPVisuals()
	for _, ESPUI in ipairs(ESPFOLDER:GetChildren()) do
		if ESPUI:IsA("BillboardGui") then
			local target = Players:FindFirstChild(ESPUI.Name)
			if Options.ESPDisplayType.Value == 'username' then
				ESPUI.PLAYERNAME.Text = target.Name
			else
				ESPUI.PLAYERNAME.Text = target.DisplayName
			end
			ESPUI.PLAYERNAME.Visible = true
			ESPUI.BOTTOMTEXT.Visible = Toggles.ESPBottomText.Value
			if not Toggles.ESPBottomText.Value then
				bottomConnection:Disconnect()
			end
			...
			ESPUI.PLAYERNAME.TextColor3 = Options.NameTextColor.Value
			ESPUI.Armor.BackgroundColor3 = Options.ArmorBarColor.Value
			ESPUI.Health.BackgroundColor3 = Options.HealthBarColor.Value
			ESPUI.BOXUI.UIStroke.Color = Options.BoxColor.Value
			ESPUI.BOTTOMTEXT.TextColor3 = Options.BottomTextColor.Value
		end
	end
end
...
DesyncGroupBox:AddToggle('Desync',{
	Text = 'desync';
	Default = false;
...
DesyncGroupBox:AddDropdown('DesyncMode',{
	Values = {'void'};
	Default = 1;
	Mutli = false;
	Text = 'desync mode';
...
RageBotGroupBox:AddDropdown('KillAuraWhitelist', {
	...
	Multi = true;
	Callback = function()
		Options.KillAuraBlacklist:SetValue(nil)
		KillAuraBlacklistTable = {}
	end,
})

RageBotGroupBox:AddDropdown('KillAuraBlacklist', {
	SpecialType = 'Player',
	Text = 'blacklist',
	Multi = true;
	Callback = function()
		Options.KillAuraWhitelist:SetValue(nil)
...
Options.KillAuraBlacklist:OnChanged(function()
	KillAuraBlacklistTable = {}
	for plr, value in next, Options.KillAuraBlacklist.Value do
		table.insert(KillAuraBlacklistTable,plr)
	end
end)
...
VisualsGroupBox:AddToggle('ESPArmorBar',{
	...
	Default = Color3.new(0.0784314, 0.745098, 0.780392),
	Callback = function(Value)
		updateESPVisuals()
	end
})

VisualsGroupBox:AddToggle('ESPBottomText',{
	Text = 'bottom text';
	Default = true;
	Callback = function()
		updateESPVisuals()
	end,
}):AddColorPicker('BottomTextColor', {
	Default = Color3.new(1, 0.878431, 0.258824),
	Callback = function(Value)
		updateESPVisuals()
	end
})

VisualsGroupBox:AddLabel('name color'):AddColorPicker('NameTextColor', {
	Default = Color3.new(1,1,1),
	Callback = function(Value)
		updateESPVisuals()
	end
...
VisualsGroupBox:AddDropdown('ESPBottomText',{
	Values = {'distance','gun'};
	Default = 1;
	Multi = false;
	Text = 'bottom text';
...
AimlockGroupBox:AddToggle('ShowRadius',{
	Text = 'show radius';
	Default = false;
})
...
AimlockGroupBox:AddSlider('CircleRadius',{
	Text = 'radius';
	Default = 200;
	Min = 50;
	Max = 800;
			...
			for _, item in pairs(workspace:GetDescendants()) do
				if item:FindFirstChild("Price") then
					...
					if ammoName and ammoName == gunName then
						purchaseItem(item,"Ammo",equippedGun.Name)
						break
					end
				end
			end
...
local function orbitAround(target)
	...
	local notKO = bodyEffects and bodyEffects:FindFirstChild("K.O") and not bodyEffects['K.O'].Value
	if targetHRP and notKO then
		if Toggles.VoidKill.Value then
			...
			HumanoidRootPart.CFrame = CFrame.new(predictedPosition) * CFrame.new(0, -6, 0)
		else
			HumanoidRootPart.CFrame = CFrame.new(targetHRP.Position) * CFrame.new(0, -6, 0)
		end
	end
end
...
TargetGroupBox:AddToggle('VoidKill',{
	Text = 'void kill';
	Default = false;
	Tooltip = 'tries to predict where the target is going to be to kill them in void';
})
...
local function handleGun(target)
	if Character:FindFirstChild("BodyEffects") and Character.BodyEffects:FindFirstChild("K.O") and not Character.BodyEffects['K.O'].Value then
		...
		if not isPurchasing and gun and gun.Parent ~= Character then
			Humanoid:EquipTool(gun)
		end
		if not isPurchasing and gun and not target.Character.BodyEffects['K.O'].Value and InventoryFolder[gun.Name].Value ~= '0' then
...
local function handlePurchases()
	if Character:FindFirstChild("BodyEffects") and Character.BodyEffects:FindFirstChild("K.O") and not Character.BodyEffects['K.O'].Value then
		...
		if not gun then
			...
			local gunName = gun.Name:match("%[(.-)%]") -- extract clean gun name
			if gun and gun:FindFirstChild("Ammo") and gunName and InventoryFolder:FindFirstChild(gunName) then
				if tonumber(InventoryFolder[gunName].Value) == 0 and gun.Ammo.Value == 0 then
					print("purchasing ammo for", gunName)
					...
					if #targets == 1 then
						...
						if target and target.Character and target.Character:FindFirstChild('BodyEffects') then
							...
							if gun then
								...
								elseif not isPurchasing and gun and gun:FindFirstChild("Ammo").Value == 0 and InventoryFolder[gun.Name].Value ~= '0' and not target.Character.BodyEffects['K.O'].Value then
									void()
									actionText.Text = 'voiding...reloading gun'
								elseif not isPurchasing and target.Character.BodyEffects["Dead"].Value and InventoryFolder[gun.Name].Value ~= '0' then
									void()
			...
			autoarmorloop = task.spawn(function()
				while Toggles.AutoArmor.Value do
					...
					if Character:FindFirstChild("BodyEffects") and not Character.BodyEffects['K.O'].Value then
						...
						elseif Options.ArmorType.Value == 'medium armor' and Character:FindFirstChild("BodyEffects").Armor.Value < 100 then
							originalCharacterPos = HumanoidRootPart.CFrame
	...
	while task.wait() do
		...
		if state then
			if not AimlockAimTarget or not AimlockAimTarget.Character or not AimlockAimTarget.Character:FindFirstChild("Head") then
				AimlockAimTarget = GetClosestPlayerToRadius(Toggles.AimlockWallCheck.Value)
...
task.spawn(function()
	while task.wait() do
		...
		if state then
			...
			local nearestToMouse = GetClosestPlayerToMouse()
